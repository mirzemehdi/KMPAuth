...
**Registration:** apps register once at startup — `KMPAuth.registerBackendProvider(FirebaseAuthBackend)`; swapping backends (e.g. Supabase) is that one line. The states living inside `kmpauth-firebase-core` (apple/github/microsoft/oauth/phone) and the deprecated 2.x containers additionally self-register the Firebase default lazily, preserving 2.x zero-config — but the generic states in core/provider modules cannot (no firebase dependency), which is why explicit registration is the documented default. First registration wins; `replace = true` to swap. `FirebaseAuthBackend` is an `expect object`; its wasm actual fails everything with a clear reason.
