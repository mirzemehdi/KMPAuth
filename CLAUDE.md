...
| `:backends:firebase:kmpauth-firebase-core` | `FirebaseAuthBackend` (default backend) with per-platform engines: GitLive firebase-auth on android/ios/js, **Firebase Auth REST API on JVM** (`FirebaseRestAuthEngine`, JDK HttpClient + kotlinx-serialization-json runtime, in-memory session - GitLive's java-sdk has no auth, #204). Plus the web-flow/platform-bound auth states `rememberAppleAuthState`/`rememberGithubAuthState`/`rememberMicrosoftAuthState`/`rememberOAuthState`/`rememberPhoneAuthState`. Its iOS Apple flow delegates to `:providers:kmpauth-apple` | core, apple |
