...
- **iOS dependencies via SwiftPM** (`swiftPMDependencies {}` DSL, Kotlin 2.4+): GoogleSignIn-iOS (google), facebook-ios-sdk products `FacebookCore`/`FacebookLogin` (facebook), firebase-ios-sdk as a closed range 12.17.0…12.999.999 (firebase; SPM `from` would cap the major and conflict with consumers ahead — GitLive was built against the lower bound). No CocoaPods anywhere. `swiftPMImport.dev.gitlive.firebase.auth.*` imports in kmpauth-firebase iosMain are GitLive's **bundled cinterop** (FIRAuth* types; was `cocoapods.FirebaseAuth.*` before GitLive 3) — never rewrite them. Stale SPM state breaks builds after version bumps: prune `.swiftpm-locks` pins and delete `swiftPMCheckout` dirs, they regenerate.
