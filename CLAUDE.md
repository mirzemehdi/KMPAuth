...
- **No Ktor client anywhere.** `kmpauth-core` ships no HTTP client — leaking one forced a Ktor version on consumers and broke their own Ktor setup (#78). Ktor appears only as `ktor-server-*` in `kmpauth-google`'s **jvmMain**, for the desktop OAuth loopback. Do not add a Ktor client dependency without a consumer for it. SPM cinterop import namespaces derive from the full gradle path (e.g. `swiftPMImport.io.github.mirzemehdi.providers.kmpauth.google.*`) — moving a module changes its imports.
